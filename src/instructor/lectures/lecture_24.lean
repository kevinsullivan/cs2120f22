...
Is it true that every ball in an empty basket 
is red? A universal quantification over an empty
set is true.
-/

inductive Ball
-- no introduction rules, no values, empty type

axioms 
  (property : Ball → Prop)
theorem  allBallsRed : ∀ (b : Ball), property b := 
begin
  assume b,
  cases b,
end
