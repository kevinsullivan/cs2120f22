...
/-
To show P → Q we first assume that P is true,
which is to say that we have a proof (p : P).
Then in this context, what remains to be proved
is Q. If we can prove Q then we've proved that
if P is true then Q is true: P → Q.
-/

example : ∀ (n : ℕ), n = 0 → n + 1 = 1 :=
begin
  assume n,
  assume nz,
  rw nz,
...
begin
  assume h,
  cases h with zez oeo,
  -- have oeo := and.elim_right h,
  exact oeo,
end
...
example : ∀ (Ball : Type) (Red : Ball → Prop),
          (∀ (b : Ball), Red b) → 
          (∃ (x : Ball), true) → 
          (∃ (x : Ball), Red x) :=
begin
  assume Ball Red,
  assume all_balls_red,
  assume some_ball,
  cases some_ball with w pf,
  apply exists.intro w _,
  exact all_balls_red w,
end
